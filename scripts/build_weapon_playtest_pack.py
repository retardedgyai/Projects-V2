...
Greatswords/daggers/staves/maces have four original Tier appearances; other families share art.
"""
import hashlib
import json
...
def assemble(base):
    files = dict(base)
    additions = []
    for pack in (BLADES,SPECIALISTS,TIERS,DAGGERS,STAVES,MACES):
    ...
    for family in FAMILIES:
        ...
        for tier in range(1,5):
            selected_key=f'{family}_t{tier}' if family in ('greatsword','dagger','staff','mace') and tier>1 else family
...
def build():
    ...
    build_blades(); build_specialists(); build_tiers(); build_daggers(); build_staves(); build_maces()
    base = installed_pack()
    ...
    report = {'status':'opt-in weapon art playtest snapshot; not automatically active',
        ...
        'distinct_tier_families':['greatsword','dagger','staff','mace'],
