...
Greatswords/daggers/staves/maces/bows have four Tier appearances; tome/astrolabe share art.
"""
import hashlib
import json
...
from build_staff_tiers import build as build_staves, PACK as STAVES
from build_mace_tiers import build as build_maces, PACK as MACES
from build_bow_tiers import build as build_bows, PACK as BOWS
...
def assemble(base):
    files = dict(base)
    additions = []
    for pack in (BLADES,SPECIALISTS,TIERS,DAGGERS,STAVES,MACES,BOWS):
    ...
    for family in FAMILIES:
        ...
        for tier in range(1,5):
            selected_key=f'{family}_t{tier}' if family in ('greatsword','dagger','staff','mace','bow') and tier>1 else family
...
def build():
    ...
    build_blades(); build_specialists(); build_tiers(); build_daggers(); build_staves(); build_maces(); build_bows()
    base = installed_pack()
    ...
    report = {'status':'opt-in weapon art playtest snapshot; not automatically active',
        ...
        'distinct_tier_families':['greatsword','dagger','staff','mace','bow'],
