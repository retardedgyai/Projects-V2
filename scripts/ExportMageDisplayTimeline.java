...
class ExportMageDisplayTimeline extends CheckNativeDisplayInterpolation {
    ...
    public static void main(String[] args) throws Exception {
        require(args.length==2,"Supply actual server timeline and output path");
        ...
        for(var raw:JsonParser.parseString(Files.readString(Path.of(args[0]))).getAsJsonArray()) {
            ...
            if(!scene.get("id").getAsString().equals("meteor"))continue;
            ...
            for(var source:scene.getAsJsonArray("frames")) {
                ...
                for(var old:previous.entrySet()) if(!current.containsKey(old.getKey())) {
                    ...
                    if(p.get("interpolation").getAsInt()==1)
                        require(vector(p.getAsJsonArray("scale")).lengthSquared()<1e-7,"Moving contour removed before zero-scale target");
            ...
            result.add(Map.of("id","meteor","name",scene.get("name").getAsString(),"frames",frames));
        }
        require(checked>=5,"Missing moving Meteor contours in actual metadata");
        Files.writeString(Path.of(args[1]),new Gson().toJson(result));
        System.out.println("PASS: "+checked+" moving contours replayed through native Display; zero-scale target before removal. No GPU/network claim.");
    }
}
