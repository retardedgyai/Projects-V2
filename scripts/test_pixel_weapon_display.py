...
def export_native_contract():
    ...
    for tier in JOBS:
        ...
        for path in sorted((ASSETS/'models/item/weapons').glob(f'pixel_{tier_key(tier)}*.json')):
            cases.append({'path':str(path),'grip':grip,'targets':TARGETS})
    from build_dagger_tiers import JOBS, ASSETS, model_data, tier_key
    for tier in JOBS:
        _,textures,entry,_,_=model_data(tier)
    ...
    for tier in JOBS:
        ...
        for path in sorted((ASSETS/'models/item/weapons').glob(f'pixel_{tier_key(tier)}*.json')):
            cases.append({'path':str(path),'grip':grip,'targets':TARGETS})
    from build_mace_tiers import JOBS, ASSETS, model_data, tier_key
    for tier in JOBS:
        _,textures,entry,_,_,_=model_data(tier)
        grip=grip_point('mace',entry,textures).tolist()
        for path in sorted((ASSETS/'models/item/weapons').glob(f'pixel_{tier_key(tier)}*.json')):
            cases.append({'path':str(path),'grip':grip,'targets':TARGETS})
    from build_bow_tiers import JOBS, ASSETS, model_data, tier_key
    for tier in JOBS:
        _,textures,entry,_,_,_=model_data(tier)
        grip=grip_point('bow',entry,textures).tolist()
        for path in sorted((ASSETS/'models/item/weapons').glob(f'pixel_{tier_key(tier)}*.json')):
            cases.append({'path':str(path),'grip':grip,'targets':TARGETS})
