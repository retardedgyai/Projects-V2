...
def contour(clip, frame, accent=False):
    ...
    if clip in ('stone_break','ultimate_rift'):
        ...
        for lane,angle in enumerate((-.82,-.32,.13,.61)):
            ...
            for i,u in enumerate(np.linspace(tail,front,22)):
                ...
                pts.append((x,y));widths.append((.55 if huge else .36)*(1-u*.55)*(1-tail))
            ribbon(g,pts,widths,accent)
        for i in range(6 if huge else 5):
            if accent: continue
            age=t-i*.65
            if not 0<=age<17: continue
    ...
    if clip=='cut_thread':
        ...
        for l in range(2):
            ...
            ribbon(g,pts,[.55*(1-age/15)]*22,accent)
        return g
    if clip=='reversal':
    ...
    if clip in ('pierce_shell','point_load'):
        ...
        for side in (-1,1):
            ...
            for lane in range(2):
                ...
                if not 0<=age<18: continue
                u=age/18
                head=5+(1-math.exp(-age*.55))*9 if not load else 5+(1-u)*7
    ...
    if clip in ('guard_edge','parry_metal'):
        ...
        if not contact:
            for i in range(3):
                ...
                u=t/19
                facet(g,(6+i,3+i*3+u*1.5),.32*(1-u),2.6*(1-u),accent,lean=.25)
        else:
            ...
            for i,a in enumerate((-.35,1.1,2.7,3.45,4.45,5.7)):
                ...
                ribbon(g,pts,[(1.2 if i in (0,3) else .4)*(1-u)*(1-age/16) for u in np.linspace(0,1,18)],accent)
        return g
    ...
    if clip=='standard_foot':
        ...
        for i in range(4):
            ...
            facet(g,(8+math.sin(a)*r,8+math.cos(a)*r),.65*(1-u),1.5*(1-u),accent)
        return g
