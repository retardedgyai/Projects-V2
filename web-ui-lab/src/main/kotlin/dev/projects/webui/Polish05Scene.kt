...
class Polish05Scene(private val kit: Path, spriteMap: Path) {
    ...
    fun forge(model: Polish05PreviewModel, note: String = "消費内容を確かめてから、鍛造を始めます。", modal: Boolean = false,
        ...
        if(modal) {
            ...
            node("modal-panel",470,176,500,568,bg="#202826",depth=7)
            node("modal-title",509,211,424,43,"この装備を強化しますか？",size=24.0,color="#ebdfc4",depth=8)
            node("modal-gear",510,286,420,80,"${selected.name}  +${selected.level} → +$next",size=23.0,depth=8)
            node("modal-attack",510,378,420,52,"物理攻撃  ${selected.power} → ${selected.power+selected.step}",size=18.0,depth=8)
            node("modal-cost",510,459,420,90,"陽鉱の塊 ${cost.materials.getValue(Material.ORE)}  ·  虚晶の欠片 ${cost.materials.getValue(Material.CRYSTAL)}  ·  ${number(cost.silver)} 銀貨",size=14.0,depth=8)
            node("modal-risk",510,550,420,67,if(snapshot.catalyst)"触媒で成功率100%" else "成功率80%。失敗時も素材と銀貨は消費します。",size=13.0,bg="#2c2b25",depth=8)
            node("modal-cancel",509,668,199,44,"やめる",size=15.0,bg="#26302e",action="cancel",depth=8,align="center")
            node("modal-confirm",718,668,213,44,"強化する",size=17.0,color="#211f19",bg="#d7bb7c",action="confirm",depth=8,align="center")
        }
