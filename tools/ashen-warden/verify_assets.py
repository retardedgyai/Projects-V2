...
for c in a['clips']:
    ...
    for start,end in c['windows']:
        assert previous<start<=end<c['duration'];previous=end
...
report={'status':'PASS','bones':len(a['bones']),'rigid_cuboids':len(a['parts']),'triangles':len(a['parts'])*12,
    'visible_item_displays':models,'pixel_textures':textures,'texture_size':'32x32','clips':[{k:c[k] for k in ('name','duration','active','windows')} for c in a['clips']],
