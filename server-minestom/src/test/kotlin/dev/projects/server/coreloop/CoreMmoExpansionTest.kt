...
internal fun coreExpansionRow(row: String) = row.substringBefore('\t') in setOf("survey", "profession", "buy-order", "dungeon-record", "gear-quality", "dungeon-run", "journey", "gear-base", "map-level", "class-build", "armor-part")
...
class CoreMmoExpansionTest {
    ...
    @Test fun `v6 upgrade preserves exact backup equipment and enhancement`() {
        ...
        val body = armorV10Body(original).lineSequence().filterNot(::coreExpansionRow).joinToString("\n").replaceFirst("\t10\t", "\t6\t")
