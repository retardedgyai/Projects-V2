...
class CoreLoopMenusTest {
    ...
    @Test fun `dungeon menus gate departure routes and return uses party-safe text in both pack modes`() {
        for (packed in listOf(false, true)) {
            ...
            assertTrue(f.snapshot().dungeonEntrance)
            assertEquals(listOf("一人で", "募集", "参加"), f.snapshot().buttons.filter { it.firstSlot in listOf(27, 30, 33) }.map { it.label })
            f.click(27); assertEquals(DungeonLobbyAction.Solo(1, 0), f.host.lobbyActions.single())
            f.host.lobbyActions.clear()
            f.host.current = f.host.current.copy(weaponBroken = true)
            f.menus.dungeons(f.player); f.click(27); f.click(30); assertTrue(f.host.lobbyActions.isEmpty())
