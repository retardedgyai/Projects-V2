...
internal class CoreWorldLoot(
    ...
    fun spawn(source: String, kind: CoreLootKind, deathPosition: Pos) {
        if (disposed || drops.containsKey(source)) return
        ...
        val summary = if (stones.isEmpty()) "魔導の粉 +${CoreAffixCatalog.lootDust(kind)}  / 戦利品券 +${CoreAffixCatalog.lootTokens(kind)}"
        ...
        val label = Entity(EntityType.TEXT_DISPLAY).apply {
            ...
            setInstance(instance, position.add(0.0, 0.75, 0.0))
        }
    ...
    private fun collect(drop: Drop): CompletableFuture<CoreTransactionResult> {
        ...
        val future = reward(CoreAction.AffixLoot(run.id, drop.source, drop.kind))
        drop.future = future
                ...
                if (error == null && result?.successful == true) {
                    ...
                    if (!disposed && owner.isOnline && owner.instance === instance) {
                        ...
                        owner.sendMessage(CoreLoopItems.text(if (drop.stones.isEmpty())
                            "戦利品回収：魔導の粉 +${CoreAffixCatalog.lootDust(drop.kind)} / 戦利品券 +${CoreAffixCatalog.lootTokens(drop.kind)}"
                            else "刻印石を${drop.stones.size}個回収！ 手帳 → 魔導工房で装備に刻めます。", NamedTextColor.GOLD))
                        inventoryChanged()
                    }
    ...
    fun dispose() { disposed = true; drops.values.forEach { d -> d.entities.forEach { it.remove() } }; drops.clear() }
}
