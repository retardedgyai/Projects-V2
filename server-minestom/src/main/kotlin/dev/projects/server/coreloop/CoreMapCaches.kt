...
internal class CoreMapCaches(
    ...
    init {
        positions.forEachIndexed { index, desired ->
            val position = QuestCombatPlacement.resolve(instance, desired)
            val chest = Entity(EntityType.ITEM_DISPLAY).apply {
                ...
                setInstance(instance, position.add(0.0, 0.5, 0.0))
            }
            val label = Entity(EntityType.TEXT_DISPLAY).apply {
                ...
                setInstance(instance, position.add(0.0, 1.6, 0.0))
            }
