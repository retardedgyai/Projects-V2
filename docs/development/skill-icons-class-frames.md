...
Creatorの実プレイ指摘を受け、固有ゲージ不足時にアイコン中央を隠していた「RP」を廃止。
`CoreHudLayout.skillVisual`が暗いフレームと空の中央表示を返すため、元のスキル絵を見分けられる。
ゲージとマナが同時に不足していても中央に文字を出さない。
クールダウン残秒、未解放の鍵、マナだけが足りない時のMPは従来どおり。
`CoreHudLayoutTest`で全70アイコンの中央揃えと、中央文字glyphが送られないことを検証する。
変更の入口・不具合時の確認先も同クラス。稼働中のゲームは止めず、反映は次回サーバー起動時。
検証結果：HUD/UIの16テスト成功（失敗0・エラー0）、distZip成功、pack構造検証成功。
pack本体の内容SHA256は変更前と同一。文字を重ねないサーバー側表示変更だけで成立する。
