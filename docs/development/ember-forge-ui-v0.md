...
- 工房の大枠・色・フォントは維持。確定は右下3枠の「強化する」等の動詞表示に統一。
  素材不足時は赤い「強化不可」、上限時は暗色「最大強化」。いずれも実行callbackなし。
- 必要素材は「名前 ×必要数」と、緑の「足りる 所持N」/赤の「あとN個 所持N」。
  色だけでなく言葉と正確な不足数で判別。触媒使用時の5種類も13行以内に収める。
- 精製・制作・補給にも同じ費用・確定表示。「素材へ」は「入手先」に変更。
- 倉庫だけ通常のCHEST_6_ROW背景へ。上段Tier、中央9〜44の36枠に実物スタック、
  下段戻る・ページ・工房。装飾は上下の茶色ガラスと選択Tierの金インゴットに限定。
- 所持品だけを表示、正式名と正確な総数はhoverに保持。見た目のスタック数は最大64。
  1枠を押すと該当の精製/刻印/ボス選択へ進む。ここでは消費しない。
- これはledger倉庫の閲覧・利用導線であり、物理アイテムの預け入れ/持ち出し機能は追加しない。
  shift・swap・drop等は引き続きキャンセル。探索中は工房へのショートカットを無効化。

実装中心は `CoreLoopMenus.kt`。費用は `CoreForgeSummary.materials` を投影し、
クリックを `CoreMenuInventory` で受けて既存のrevision付きledger操作へ渡す。
不具合時は `CoreLoopMenusTest` の混在不足/充足遷移、倉庫実スタック/転送抑止を先に確認。
...
- メニュー/倉庫/費用/クリック管理の63テスト成功、`installDist`成功（最終25秒）。
- 13画面strict警告0。混在不足・触媒5素材のケースを追加し、実装プレビューで確認。
  倉庫はCanvasを描画しないため、旧倉庫PNG/監査JSONを削除（Git履歴から復元可）。
- アセット検証288件成功。内容SHA256 `93cbf739e70ff14c55ff183c9f5eb8e17d9df4d80750e38ec1b8f1bf357ac91e`。
- 新しい倉庫の実スタック/正確な総数/空欄/通常タイトル/パック有無/探索中制限を検査。
  再起動と実ゲームでの手動確認はまだ行っていない。
- 次の小改善候補：強化結果を画面内に残す（現状はチャット・音と数値更新）、
  通常/触媒の効果差をhoverせず比較できる表示。全面配置変更は不要と判断。
