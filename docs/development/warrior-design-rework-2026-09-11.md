...
- Creatorの最新指示により、今回の作画の到達判定はAgentが戦士全体のプレビューで行う。
  テスト成功を作画の合格とは読み替えず、リファレンスと並べて形・強弱・動き・UIを評価する。
- 比較画像、実アセット検査、範囲/向き/寿命/描画契約を残す。
  Creatorによる実機操作/feel判定は引き続き別工程だが、今回の作画作業をそこで止めない。
...
## 戦士全体のプレビュー判定（Creator指定の完了基準へ更新）

Creatorより「Previewを見てMatE水準に達したと思ったらでよい。そこまで戦士全体で続ける」と指定。
この追記以降、**作画の到達判定はAgentの比較レビューで行う**。過去節の「実機判断が来るまで
作画完了不可」は今回の完了ゲートではない。AGENTSのManual Smoke操作/最終feelは引き続きCreatorの領域。
ゲームは起動/操作せず、プレビューの描画と本編クライアントの描画を同一と主張しない。

### 参照からの再評価と修正

- X本文の403で止まらず、X公式の埋め込みプレイヤーで
  [MatE Hollow Monarch](https://x.com/MatE312001/status/2052458279125610949)の4.26秒GIFを確認。
  0/0.4/0.8/1/1.2/1.4/2/2.2/2.4/2.8/3/3.2/3.4/3.8/4.1秒のブラウザー描画を採取。
  武器周囲への集中、地面の細い境界、剣を立てた放出時の鋭い長い形、離れる細片を比較した。
  制作手法や元モデルのトポロジーを断定せず、雷/金色そのものを戦士にコピーしない。
  Solar Scepterは今回はサムネイルまでで、再生ボタンがX本体へ遷移して失敗したため動画確認済みとしない。
- 全技能を比較すると、前版の雄叫びは均等な横帯、返しの補助形は裂傷と同方向、
  地表の圧力も同じfanで、支援範囲は大きな四角の列に見えた。これを具体的な修正対象にした。
- 雄叫び：横帯を廃し、胸前から広がる大小6つの開いた波片。主役の声のechoを隠さない位置へ。
- 返し：専用counter輪郭を追加し、裂傷と逆方向の先端/消散へ。奥義中段にも接続。
- 地表：専用ground輪郭で、中央の強い前方片と短い斜めの2片に分離。ground配置補正も有効。
- 地砕き/天断の立ち上がり：均等な7本を廃し、強弱を付けた非対称5本へ。
  斬り上げは別の3本の上昇形。旋風も大小と離れる速度を付け、3段の差を維持。
- 本体の鋼白は既存の主役斬撃と同じtintへ。アクセントの紅は狭い部分だけ。
- 範囲：テクスチャは変えず、ネイティブ面の刻みを1/16モデル単位に。
  7モデル単位の中心線・固定の実radius・12tickの寿命を保持し、幅広い四角でなく細い輪郭にした。
- 受け流しのCONTACTに残っていた、旧式の大きな星を遅延して二度出す表示を廃止。
  承認済み斬撃と同じnative contactを一度だけ9tick表示。旗/雄叫びの架空CONTACTは空にする。
  全技能を一律「CONTACTも必ず1個以上」としていたテストを、これら2支援技だけは0が正しい契約へ更新。

### プレビューの信頼性と全体判定

- カメラ背後の頂点を手前へclampしていた簡易投影をnear-plane clippingへ修正。
  旗の背後へ飛ぶ布片が画面全面の赤い面に化けるのは投影器の問題だったため、承認済み旗を変更しない。
- 不透明な1texelインク面は同じ色のポリゴンとして直接描画し、全画面warpの反復を省く。
  演出を別の絵で置き換えたり、追加bloom/滑らかな補間で完成度を装ったりしていない。
- 実ItemDisplay出力を、全10技能＋AA3段、接触なし/ありまで拡張。
  接触例は固定位置でCONTACTを入力した**描画確認用ケース**であり、録画したゲーム内命中ではない。
  AAはGreatswordVfxと同じskill定義＋scene overrideを使う。最初のテストでAAをskill iconにして
  motif未登録エラーが出たが、この本番と同じ分離へ直した。攻撃入力/判定は改変していない。
- 発動時点に揃える`--relative-ticks`も追加。単純な絶対時刻では、始動18tickの奥義と即時AAを
  並べた時に奥義だけ何も出ていないという不公平な比較になるため、通常時間の連続GIFと併用する。

| 範囲 | プレビュー上の判定根拠 | 作画判定 |
|---|---|---|
| AA3段/踏み込み | 承認済み刃と個別消散を維持。即時AAの本番契約は不変 | PASS・保持 |
| 裂傷/返し | 短い切断と広い逆向き切断。補助輪郭も逆方向へ分離 | PASS |
| 破城 | 横の斬撃でなく前へ伸びる芯と左右に抜ける気流 | PASS |
| 地砕き | 縦の主役、大小の噴出、前方の地表片、離れる余韻 | PASS |
| 旋風 | 3段の輪郭/高さ/破片に差があり、単一完成画像の回転ではない | PASS |
| 天断 | 上昇→横返し→縦の断撃の順序と、最後だけ重い地表/噴出 | PASS |
| 受け流し | 原画を保持した剣の構え。一度だけの短い接触光 | PASS |
| 雄叫び | 胸前の圧縮と外へ抜ける開いた声の波。斬撃/ダメージの偽演出なし | PASS |
| 不屈の旗 | 承認済みの布/固定上端/風、細い実radius輪郭、分離する裾と余韻 | PASS・保持 |
| 編成UI/奥義/HUD | 同一の承認済み字体/アイコン。キー/選択/確定を分離、4状態の絵と文字の非重複 | PASS・保持 |
| tooltip/印 | 役割→用途→実係数/費用/状態の順序を維持。印は既存アイコン＋実残秒、名前とは別の位置 | 保持・契約検証 |

**Agentの判断：Creatorが指定したプレビュー基準で、戦士全体の作画は目標水準に達したとして合格。**
これはMatE本人の評価や客観的な同一品質の証明ではなく、上記の比較に基づく制作上の判断。
元作品を複製したという意味でもない。サーバー/RPのみを維持し、ゲーム入力/数値/他職業/日本語原画を変更しない。

確認用：`.tools/warrior-preview-{aa,cuts,heavy,support}.gif`、
`.tools/warrior-reviewed-menu.png`、`warrior-reviewed-ultimates.png`、`warrior-reviewed-hud.png`。
GIFは実装の20Hzモデル列で、元モデルのUV/座標/寿命を使用。ゲーム画面・通信遅延・実機FPSの録画ではない。
メニューは実Inventoryからのcanvas/flat item、HUDは実Component/fontを合成。
hover/バニラ3D item/worldは描画対象外であり、それらの実機再現性を今回の作画判定に混同しない。

主な入口：`CoreWarriorFlourish`と`build_warrior_flourish.py`（技の補助形）、
`CoreWarriorSupportChoreography`（受け流しCONTACT）、`build_warrior_companions.py`（範囲線）、
`preview_skill_choreography.py`（投影/再生確認）。操作と実数値は従来のサーバー経路のまま。

### 最終検証

- 全サーバーテスト839件成功（失敗/エラー/スキップ0）。Python戦士系24件＋凍結AA/dash4件成功。
- 最初の全839件では、支援CONTACTを空にしたことに対する旧期待の1件のみ失敗。
  2支援技に限って空を要求する厳密な期待へ変更し、全体を再実行して成功を確認した。
- pack検査：12,049 assets / 50,695 private glyphs / グローバルフォント上書きなし。
  content SHA256 `a2f7945bfb8fa2504a25204bb4c8e48801ba1faaf00f2412ebcc973be93ee463`。
- 技能/奥義メニューはアイコン13/7枚、欠落0、文字省略/欠字警告0。HUDは4状態を実Componentで合成し目視確認。
- 配布ZIPのJAR、変更した描画クラス3件、全packファイルがbuild/classesとソースにbyte単位で一致。
  ZIP SHA256 `b3edaaec6fbcd427f22a72d02216e6de36c1e04b928d72923fe6c63654bcd1a9`。
- `.tools/`の参照スクリーンショット/プレビュー、`.kotlin/`、build出力はcommitしない。
  main/client-fabric/protocol、承認済み大剣/AA/旗/日本語字体/技能アイコン原画は変更なし。
  起動済みのゲームとインストール済み実行環境へは適用せず、次の起動依頼時にこの配布物を反映する。
