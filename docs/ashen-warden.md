...
刀身のローカルYが刃幅、Xが平面の法線、Zが長さ。横方向の刃速度へY軸を合わせる。連撃の間は平行移動した回転基準を使って刃の向きをつなぎ、肘の伸展による手首の急反転を防ぐ。剣だけを手から独立回転させない。
...
`review_sword.py`で全8攻撃を固定カメラから518フレーム・25.9秒で実レンダーする。引数が出力ディレクトリだけなら、剣側と側面から主要姿勢をレンダーする。プレビューにゲーム内trail・音は含まない。
