...
| vault_slam | 72 tick | 41–44 | 低い溜め→膝を畳んだ前方宙返り→着地の叩き斬り |
...
踏み込み斬りを地上の低い斬撃へ変更し、大回転・連続回転・前方宙返りの3動作を追加。その後のF2画像で、跳躍技の構えが胸・頭へ埋まり、刃の平面を当てる振り方になっていることが判明したため、6攻撃の腕と握りを再制作した。
右手の軌道を肩の外側と身体の前へ移し、肩の引き・肘の屈曲・前方への伸展・振り抜き・回復を別々にキー化。上腕は胸のローカル空間でも動き、回転技で固定した腕を胴体に載せて回す状態を解消した。左腕と両肩の動き、既存の足・腰・胸の時間差を合わせる。
実際の刀身はローカルYが刃幅、Xが平面の法線、Zが長さ。刃の中央付近の横方向速度へY軸を向ける。予備動作中に向きを合わせ、攻撃後に自然な握りへ戻す。空中の反転では関節とともに刃の面も運び、剣だけを独立回転させない。
...
全clipの握り、腕の接続、接地、6攻撃の軌道、連続回転の別々の命中区間、踏み込みの非浮遊、宙返りの反転・着地、単発実演、移動の連続性、切り替え、vanilla描画時の回転補正を含む23件のボス専用テストを実行。追加の3件は刃の向き、刀身・鍔と胸・頭・腹の中心領域との交差、胴体に対する上腕の動きと肘の屈伸を検証する。中心領域の検査は鎧の突起・布を含む全メッシュの完全な衝突検査ではない。
...
握りと刃の修正版は`review_sword.py`で6攻撃を剣側のカメラから360フレーム・18秒で実レンダーする。引数が出力ディレクトリだけなら、剣側と側面から主要姿勢をレンダーする。
